...
class GenericNeuralNet(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.logits,self.model_to_be_saved=self.load_model(self.input_placeholder)
        ...
        self.reshape=lambda a:np.reshape(a,(-1))
        ...
        self.checkpoint_file = os.path.join(self.train_dir, "%s_checkpoint" % self.model_name)
...
#            print(cur_pos.shape)
    ...
    def fill_feed_dict_with_one_ex(self, data_set, target_idx):
        ...
        return feed_dict
    def fill_feed_dict_with_attacked_pts(self,attacked_data_set,target_idx):          
        input_feed  = attacked_data_set.train.x[target_idx,:].reshape(1, -1)
        labels_feed = attacked_data_set.train.labels[target_idx].reshape(-1)
    ...
    def train(self, num_steps,iter_to_switch_to_batch, 
        ...
        for step in xrange(num_steps):
            ...
            if (step + 1) % 100000 == 0 or (step + 1) == num_steps:
                ...
                print("check first print----------------------------------------------------------------")
                if verbose: self.print_model_eval() 
                params_test=sess.run([self.params])
                print(params_test[0][1])
                self.model_to_be_saved.save(self.checkpoint_file+"KERAS_"+str(step))
                print("check second print----------------------------------------------------------------")
                if verbose: self.print_model_eval()                
                params_test2=sess.run([self.params])
                print(params_test2[0][1])
    ...
    def get_train_mom_op(self, total_loss, global_step, learning_rate=0.001):
        """
        Return train_momentum_op
        """
    ...
    def minibatch_hessian_vector_val(self, v):
        ...
        for i in xrange(num_iter):
            ...
            self.update_feed_dict_with_v_placeholder(feed_dict,v)
...
#        hessian_vector_val = [a + self.damping * b for (a,b) in zip(hessian_vector_val, v)]
        hessian_vector_val = [self.reshape(a) + self.damping * b for (a,b) in zip(hessian_vector_val, v)]
...
#        print('minibatch_HV_value_calculation took  %s sec' % duration)
    ...
    def update_feed_dict_with_v_placeholder(self,feed_dict,vec):
        for pl_block,vec_block in zip (self.v_placeholder,vec):
            feed_dict[pl_block]=np.reshape(vec_block,pl_block.get_shape())
        return feed_dict
    ...
    def get_cg_callback(self, v, verbose):
        ...
        def cg_callback(x):
            ...
            for i in range(np.shape(v)[0]):
                v[i]=v[i].reshape(-1)
                train_grad_loss_val[i]=train_grad_loss_val[i].reshape(-1)
            ...
            if verbose:
                ...
                print("Iteration number:",self.maxiter_idx)
                self.maxiter_idx+=1
        return cg_callback
    ...
    def get_inverse_hvp_cg(self, v, verbose):
        ...
        for i in range(np.shape(v)[0]):
            v[i]=v[i].reshape(-1)
        print("all trainable parameters in hvp: ", np.concatenate(v).shape)
    ...
    def get_influence_on_test_loss(self, test_indices, train_idx, 
        ...
        loss_type='normal_loss',dataset_type="regular"):
        ...
        start_time = time.time()
        if dataset_type=="attacked":
            attacked_dataset=train_idx
            num_to_remove = attacked_dataset.train.x.shape[0]
            predicted_loss_diffs = np.zeros([num_to_remove])
            for counter in range(num_to_remove):            
                single_train_feed_dict = self.fill_feed_dict_with_attacked_pts(attacked_dataset,counter)      
                train_grad_loss_val = self.sess.run(self.grad_total_loss_op, feed_dict=single_train_feed_dict)

                for i in range(len(inverse_hvp)):
                  inverse_hvp[i]=inverse_hvp[i].reshape(-1)
                  train_grad_loss_val[i]=train_grad_loss_val[i].reshape(-1)
